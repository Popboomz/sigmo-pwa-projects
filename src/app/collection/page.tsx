...
export default function CollectionPage() {
  ...
  const getProductLineColor = (lineKey: string) => {
    switch (lineKey) {
      case 'core':
        return {
          bg: 'brand-pill brand-pill-core',
          text: 'text-[var(--y2k-blue)]',
          border: 'border-[rgba(122,168,255,0.18)]',
          hover: 'hover:border-[rgba(122,168,255,0.28)]',
        };
      case 'x':
        return {
          bg: 'brand-pill brand-pill-x',
          text: 'text-[var(--y2k-lilac)]',
          border: 'border-[rgba(213,192,255,0.22)]',
          hover: 'hover:border-[rgba(213,192,255,0.32)]',
        };
      case 'lab':
        return {
          bg: 'brand-pill brand-pill-lab',
          text: 'text-[var(--y2k-pink)]',
          border: 'border-[rgba(255,182,223,0.22)]',
          hover: 'hover:border-[rgba(255,182,223,0.32)]',
        };
      default:
        return {
          bg: 'brand-pill brand-pill-core',
    ...
    <div className="min-h-screen mesh-bg y2k-shell relative overflow-hidden">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 pt-32">
        ...
        <div className="text-center mb-16">
          ...
          <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full y2k-chip">
            <Shield className="w-4 h-4 text-[var(--y2k-blue)]" />
          ...
          {productLineKeys.map((lineKey, index) => {
            ...
            return (
              <div key={lineKey} className="scroll-mt-32 product-line product-line--{lineKey} animate-on-scroll" style={{ animationDelay: `${index * 0.2}s` }} id={`line-${lineKey}`}>
                ...
                <div
                  className={`rounded-2xl p-8 md:p-12 mb-8 transition-all duration-300 hover:shadow-md premium-card y2k-panel`}
                  style={{
                    background: lineKey === 'core'
                      ? 'linear-gradient(135deg, rgba(126,232,255,0.18) 0%, rgba(108,167,255,0.08) 100%)'
                      : lineKey === 'x'
                      ? 'linear-gradient(135deg, rgba(213,192,255,0.18) 0%, rgba(255,182,223,0.08) 100%)'
                      : 'linear-gradient(135deg, rgba(255,216,168,0.18) 0%, rgba(255,182,223,0.08) 100%)',
                    border: `1px solid ${lineKey === 'core' ? 'rgba(122,168,255,0.18)' : lineKey === 'x' ? 'rgba(213,192,255,0.22)' : 'rgba(255,182,223,0.22)'}`,
                  }}
                >
                  <div className="flex items-start gap-4">
                    <div
                      className="w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 brand-tile"
                    >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <Badge className={`rounded-full px-4 py-1.5 flex items-center gap-1.5 ${colors.bg}`}>
                          {productLine.badge}
                        </Badge>
                      </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {productLine.products.map((product, productIndex) => {
                    ...
                    return (
                      <div
                        key={product.id}
                        className={`card-base bg-white overflow-hidden relative group animate-on-scroll premium-card y2k-panel`}
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{
                            background: lineKey === 'core'
                              ? 'linear-gradient(180deg, #7ee8ff, #6da7ff)'
                              : lineKey === 'x'
                              ? 'linear-gradient(180deg, #d5c0ff, #ffb6df)'
                              : 'linear-gradient(180deg, #ffd8a8, #ffb6df)',
                          }}
                        />
                        ...
                        <div className="p-6">
                          <div className="flex justify-between items-start mb-3">
                            <Badge className={`rounded-full px-4 py-1.5 flex items-center gap-1.5 text-xs font-semibold ${colors.bg}`}>
                          ...
                          <Button
                            ...
                            className="w-full rounded-xl border-2 border-border text-secondary-foreground font-medium text-sm y2k-ghost hover:scale-102 hover:shadow-lg transition-all duration-300 py-3 group-hover:shadow-md"
                          >
